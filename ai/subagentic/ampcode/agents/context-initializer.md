...
❌ **NO embedded definitions** - Don't duplicate ~/.config/amp/agents/ or ~/.config/amp/skills/ content
...
## Agent System
**IMPORTANT**: Global agent system active from `~/.config/amp/AGENTS.md`.
...
- See `~/.config/amp/AGENTS.md` for workflow patterns
...
- ~/.config/amp/AGENTS.md: Global config
...
- ~/.config/amp/agents/: Should load when @mentioned (but currently broken)
- ~/.config/amp/skills/: Should load when /invoked (but currently broken)

# Common Mistakes

**Mistake 1**: "Let me add details to help Ampcode"
...
- **Right**: Reference ~/.config/amp/AGENTS.md for agent list
...
Remember: Your job is to create the MINIMAL AGENTS.md that enables Ampcode to navigate COMPREHENSIVE KNOWLEDGE_BASE.md. Be ruthless with AGENTS.md, generous with KNOWLEDGE_BASE.md.
